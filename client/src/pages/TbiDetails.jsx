  ...
  if (error || !tbi) {
    return (
      <div className="max-w-md mx-auto py-16 text-center">
        ...
        <Link
          to="/explore"
          className="px-4 py-2 bg-navy text-white text-xs font-semibold rounded-lg shadow-sm hover:bg-navy-600 transition-colors"
        >
          Back to Explore
        </Link>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div>
        <Link
          to="/explore"
          className="inline-flex items-center space-x-1.5 text-xs font-semibold text-slate-muted hover:text-navy transition-colors"
        >
      ...
      <div className="bg-surface border border-slate-border rounded-2xl p-6 sm:p-8 shadow-card space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
          <div className="flex items-start space-x-4">
            {/* Avatar / Logo */}
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-navy-50 border border-navy-100 flex items-center justify-center font-extrabold text-navy text-2xl sm:text-3xl shrink-0 shadow-sm overflow-hidden">
              {tbi.logo ? (
                <img
                  src={tbi.logo}
                  alt={tbi.name}
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2 mb-1.5">
                ...
                {tbi.incubatorType && (
                  <span className="text-xs font-semibold text-navy bg-navy-50 px-2.5 py-0.5 rounded-full border border-navy-100">
                    {tbi.incubatorType}
                  </span>
                )}
              </div>
          ...
          <div className="flex items-center space-x-2 shrink-0">
            <button
              onClick={handleShare}
              className="p-2.5 rounded-xl border border-slate-border text-slate-muted hover:text-navy hover:bg-slate-50 transition-colors"
            ...
            <button
              ...
              disabled={savingFav}
              className={`flex items-center space-x-1.5 px-4 py-2 rounded-xl border text-xs font-semibold transition-all ${
                isFavorited
        ...
        {copied && (
          <div className="text-xs font-semibold text-teal bg-teal-50 px-3 py-1.5 rounded-lg border border-teal-200 text-center animate-in fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 text-sm">
          <div className="flex items-start space-x-3">
            <Building2 className="w-5 h-5 text-slate-muted shrink-0 mt-0.5" />
            <div>
          ...
          <div className="flex items-start space-x-3">
            <MapPin className="w-5 h-5 text-slate-muted shrink-0 mt-0.5" />
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-muted">
                Location
              </p>
          ...
          <div className="flex items-start space-x-3">
            <Mail className="w-5 h-5 text-slate-muted shrink-0 mt-0.5" />
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-muted">
                Official Email
              </p>
              {tbi.email ? (
                <a
                  ...
                  className="font-semibold text-navy hover:text-teal hover:underline transition-colors mt-0.5 block"
                >
                  {tbi.email}
                </a>
              ) : (
          ...
          <div className="flex items-start space-x-3">
            <Globe className="w-5 h-5 text-slate-muted shrink-0 mt-0.5" />
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-slate-muted">
                Website
              </p>
              {tbi.hasValidWebsite && tbi.websiteUrl ? (
                <a
                  ...
                  className="inline-flex items-center space-x-1 font-semibold text-teal hover:underline mt-0.5"
                >
